...
\appendix
\section{A. Repository and registry design}
...
\section{B. Reproduction commands}
The locked Python environment is installed with \texttt{make sync}. \texttt{make test} runs the test suite and \texttt{make lint} checks repository formatting. Individual Make targets rerun Experiment Zero, sample efficiency, generator shift, robustness, task suite, active evidence, dynamics, ablations, observables, training laws, hard halting, trajectories, and discovery. \texttt{make figures} regenerates every PNG and vector PDF from result files. \texttt{make paper} rebuilds tables, modular LaTeX, DOCX, and the PDF; \texttt{make reproduce-paper} verifies cached artifacts and performs the complete publication build.
...
\section{C. Full-data saturation}
...
\section{D. Strong-control generator shift}
...
\section{E. Active acquisition details}
...
\section{F. Unconventional dynamics}
...
\section{G. Training-law diagnostics}
...
\section{H. Hard-halting threshold selection}
...
\section{I. Statistical cautions}
...
\section{L. AI-assistance disclosure}
...
\section{M. Checklist for external replication}
\begin{itemize}
\item Freeze the primary and co-primary endpoints before model training.
\item Implement a separately authored causal generator with new factor and observation mechanics.
\item Include tuned GRU, state-space, real operator, two-channel, block-real-equivalent, and complex controls.
\item Use enough independent world seeds for stable top-level uncertainty.
\item Report ambiguity NLL, valid-set mass, calibration, and compute alongside accuracy.
\item Select hyperparameters without shifted test labels.
\item Preserve failed and superseded runs in a public registry.
\item Release code, configurations, raw summaries, and generated artifacts.
\item Prohibit clinical interpretation unless a separate governed validation program succeeds.
\end{itemize}
