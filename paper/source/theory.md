...
## Complex hypothesis-state update

Let a tokenized observation at step t be embedded as a real vector x_t. The implemented complex operator maintains h_t in C^d and applies a token-conditioned update followed by a phase-compatible nonlinearity and a magnitude-sensitive readout. Suppressing low-rank factorization and bias terms, the linear part is

{{equation:h_t = W_t h_{t-1} + U x_t|h_t = W_t h_(t-1) + U x_t.}}

Write h = a + ib and W = A + iB, with a, b real vectors and A, B real matrices. Then the same multiplication is

{{equation:\begin{bmatrix}\Re(Wh)\\\Im(Wh)\end{bmatrix}=\begin{bmatrix}A&-B\\B&A\end{bmatrix}\begin{bmatrix}a\\b\end{bmatrix}|[Re(Wh); Im(Wh)] = [[A, -B], [B, A]] [a; b].}}
...
- **Representational uniqueness:** the complex model computes a function unavailable to the exact structured real block.
- **Parameterization advantage:** the complex coordinates create a more useful inductive bias or optimization trajectory even though the represented mapped function class overlaps.
- **Architecture advantage:** the complete complex operator outperforms all parameter- and compute-matched real sequence architectures on the prespecified outcome.
...
For family f, training size n, training seed s, world w, and severity q, let A_C(f,n,s,w,q) be complex top-1 accuracy. Let R be the prespecified set of evaluated real controls. The nested effect is

{{equation:\Delta_{fnswq}=A_C(f,n,s,w,q)-\max_{r\in\mathcal{R}}A_r(f,n,s,w,q)|Delta = complex top-1 - maximum real-control top-1 in the same cell.}}
...
## Structural predictors and candidate law
...
{{equation:\widehat{\Delta}=\beta_0+\beta_1 I+\beta_2 q+\beta_3 I^2+\beta_4 Iq+\beta_5 q^2|predicted Delta = b0 + b1 I + b2 q + b3 I^2 + b4 Iq + b5 q^2.}}
