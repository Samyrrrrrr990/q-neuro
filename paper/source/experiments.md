...
1. **Comparator audit:** reconstruct the historical result and determine whether the two-channel control is algebraically equivalent.
2. **Exact-equivalence and mechanism tests:** map the complex operator into a tied real block and test forward and outcome agreement; apply destructive phase interventions only as mechanism probes.
3. **Power pilot:** estimate the number of worlds required for a minimum practical effect of 0.02 without making an outcome claim.
4. **Reduced discovery:** train five models on four independent generator families; compute cellwise best-real effects; select one law from six prespecified families.
5. **Freeze:** store coefficients, thresholds, hashes, family split, and a prohibition on using the candidate for the grand outcome.
6. **Held-out reduced confirmation:** evaluate the frozen law and architecture effect on four untouched families with new seeds.
7. **Grand preflight:** open no sealed outcome unless every readiness gate passes.
